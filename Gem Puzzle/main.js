...
let sizeFieldNumberOfHTML = 4;
const sizeFieldIndex = document.getElementsByClassName("sizeFieldIndex");

const CELL_COUNT = sizeFieldNumberOfHTML;
  ...
  for (let i = 0; i < CELL_COUNT; i++) {
    ...
    for (let j = 0; j < CELL_COUNT; j++) {
      while (row.length !== CELL_COUNT) {
        const n = Math.floor(
          Math.random() * sizeFieldNumberOfHTML * sizeFieldNumberOfHTML
        );
      ...
      for (let col = 0; col < playField[row].length; col++) {
        ...
        if (playField[row][col]) {
          ...
          ctx.fillText(
            playField[row][col],
            dx + offset / 2,
            dy + CELL_WIDTH / sizeFieldNumberOfHTML
          );
        }
      }
    }
...
// sizeField.addEventListener("click", (event) => {
//   if (event.target.tagName !== "P") return;
//   sizeFieldNumberOfHTML = event.target.getAttribute("data-size");
//   console.log(sizeFieldNumberOfHTML);
// });
